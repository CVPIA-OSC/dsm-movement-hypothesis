...
spring_run_hypothesis <- spring_run_hypothesis_raw %>%
...
spring_run_hypothesis %>%
  ...
  ungroup() %>%
  write_rds("data/spring-run-juveniles-at-chipps-clean-new-metric.rds")

# Valley-wide proportions -------------------------------------------------

# get valley total by summing across all the watersheds, preserve size_class
valley_totals <- spring_run_hypothesis %>%
  # select(-(Oct_Mar:Index)) %>%
